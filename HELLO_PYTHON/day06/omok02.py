...
class MainClass(QMainWindow, form_class):
    ...
    def myreset(self):
        QMessageBox.question(self, '새로운게임', "새로운게임을 시작합니다", QMessageBox.Yes, QMessageBox.NoButton)
        for i in range(10):
            for j in range(10):
                self.arr2D[i][j]=0
    ...
    def myclick(self):
        ...
        if d1 == 5 or d2 == 5 or d3 == 5 or d4 == 5:
            self.flagIng = False
            if self.flagWb:
                QMessageBox.question(self, '오목', "백돌승리", QMessageBox.Yes, QMessageBox.NoButton)
            else:
